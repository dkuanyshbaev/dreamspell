...
pub async fn update_seal(db_pool: &SqlitePool, seal: &Seal) -> Result<(), sqlx::Error> {
    sqlx::query(
        "UPDATE seals SET name = ?, name_en = ?, image = ?, archetype = ?, archetype_en = ?, archetype_description = ?, archetype_description_short = ?, archetype_description_en = ?, archetype_description_short_en = ?, portrait_description = ?, portrait_description_short = ?, portrait_description_en = ?, portrait_description_short_en = ?, type_description = ?, type_description_short = ?, type_description_en = ?, type_description_short_en = ? WHERE id = ?"
    )
    .bind(&seal.name)
    .bind(&seal.name_en)
    .bind(&seal.image)
